  ...
  const aggregatedPersonas = useMemo(() => {
    ...
    return baseGroups.map(group => {
      ...
      return {
        ...
        confusionPoints: personaClusters
        ...
        commonQuestions: personaClusters
          .slice(0, 3)
          .map(c => c.canonicalQuestion),
        contentSuggestions: personaClusters.length > 0
          ? personaClusters.filter(c => c.status !== 'answered').slice(0, 3).map(c => `Address question: "${c.canonicalQuestion}" in your next pitch update`)
          : [`Create targeted messaging tailored for ${personaConfig[group.persona].name}`]
      };
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        <div className="bg-card rounded-2xl border border-border/30 p-4 md:p-5 shadow-card">
          <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
            ...
            <span className="text-[11px] md:text-sm font-medium text-muted-foreground truncate">Total Audience</span>
          </div>
          <p className="text-2xl md:text-3xl font-display font-bold text-foreground">{stats.totalMembers}</p>
        </div>
        <div className="bg-card rounded-2xl border border-border/30 p-4 md:p-5 shadow-card">
          <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
            ...
            <span className="text-[11px] md:text-sm font-medium text-muted-foreground truncate">Total Questions</span>
          </div>
        ...
        <div className="bg-card rounded-2xl border border-border/30 p-4 md:p-5 shadow-card">
          <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
            ...
            <span className="text-[11px] md:text-sm font-medium text-muted-foreground truncate">Confusion</span>
          </div>
          <p className="text-2xl md:text-3xl font-display font-bold text-foreground">
        ...
        <div className="bg-card rounded-2xl border border-border/30 p-4 md:p-5 shadow-card">
          <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
            <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-success/10 flex items-center justify-center shrink-0">
              <Lightbulb className="w-4 h-4 md:w-5 md:h-5 text-success" />
            </div>
            <span className="text-[11px] md:text-sm font-medium text-muted-foreground truncate">Suggestions</span>
          </div>
          <p className="text-2xl md:text-3xl font-display font-bold text-foreground">
            {clusters.filter(c => c.status !== 'answered').length}
          </p>
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-semibold text-foreground font-display">{config.name}</h4>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-foreground font-display">{personaData.percentage}%</span>
                    </div>
          ...
          {selectedPersonaData ? (
            <>
              ...
              <div className="bg-card rounded-2xl border border-border/30 p-5 shadow-card">
                <h4 className="font-medium text-foreground mb-3 flex items-center gap-2 font-display">
                ...
                {selectedPersonaData.confusionPoints.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground italic">No confusion points detected yet.</p>
                )}
              </div>
              ...
              <div className="bg-card rounded-2xl border border-border/30 p-5 shadow-card">
                ...
                {selectedPersonaData.commonQuestions.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground italic">No common questions recorded yet.</p>
                )}
              </div>
            ...
            <div className="bg-card rounded-2xl border border-border/30 p-8 text-center shadow-card font-sans">
              <User className="w-10 h-10 text-muted-foreground mx-auto mb-3" />
              <p className="text-muted-foreground">
                Select a persona card to view detailed insights
              </p>
            </div>
          )}
        </div>
